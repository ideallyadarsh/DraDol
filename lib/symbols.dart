...
class _SymbolsState extends State<Symbols> {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        child: Column(children: [
          SizedBox(height: 10,),
          Text("Match The Symbol With the Word",style: TextStyle(fontSize: 20,color: Colors.black),),
          SizedBox(height: 5,),
          Text("Your Score:${score.length}/8",style: TextStyle(fontSize: 30,color: Colors.black),),
          SizedBox(height: 30,),
    ...
    return DragTarget<String>(
      builder: (BuildContext context,List<String> incoming,List rejected){
        ...
        if(score[words]==true){
          ...
          return Padding(
            padding: const EdgeInsets.all(4.0),
            child: Container(alignment: Alignment.centerRight,child:Text(word[words],style: TextStyle(fontSize: 17),),height: height/9, width:150,),
          );
        }
      },
  ...
  dialogue(BuildContext context){
    return showDialog(
      ...
      return AlertDialog(
        title: Text("Score=8/8",textAlign: TextAlign.center,),
        actions: [
          Column(
            children: [
